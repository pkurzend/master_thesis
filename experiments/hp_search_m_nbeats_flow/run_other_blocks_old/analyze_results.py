...
for ds_name in ['electricity_nips', 'traffic_nips', 'solar_nips', 'exchange_rate']:
    ...
    print('model_ids ', df['model_id'].unique()) # per data set, 40 models have been trained: 4 variants, each 10 repeated times
    # model_id counts from 1 to 40, hence same repeated runs cannot be identified using model_id anymore (model_id has been set wrong)
    
    # repeated runs use the same block type: 40 runs, 10 runs each of the 4 block types
    print('block_types ', df['blocks'].unique())

    # sys.exit()

    for block_types in df['blocks'].unique():
        df_temp = df.loc[df['blocks'] == block_types]
